...
export default {
  computed: {
    ...
    adminModules() { return this.modules.filter(module => module.shell === "admin-shell").sort((a, b) => Number(a.code) - Number(b.code)); },
    ...
    currentMetrics() {
      ...
      if (this.currentId === "adminSync" && this.adminSyncMetrics?.length) return this.adminSyncMetrics;
    ...
    syncProgress() {
      if (this.currentId !== "adminSync") return 100;
      const total = Number(this.selectedRow[3] || 0);
      const success = Number(this.selectedRow[4] || 0);
      if (!total) return 0;
