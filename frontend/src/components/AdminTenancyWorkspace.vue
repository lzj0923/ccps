  ...
  <section class="content-grid admin-tenancy-workspace">
    <div class="panel table-panel tenancy-list-panel">
      ...
      <div v-else class="table-wrap tenancy-table-wrap">
        <table>
          <thead><tr><th>{{ $t('legacy.t_42c950a2ce7c') }}</th><th>{{ $t('legacy.t_114246450ff0') }}</th><th>{{ $t('legacy.t_bd95f007d6d0') }}</th><th>{{ $t('legacy.t_52661b02b6a2') }}</th><th>{{ $t('legacy.t_a7ff541f4978') }}</th><th>{{ $t('legacy.t_fdeda5befee2') }}</th><th>{{ $t('legacy.t_17973bd1a231') }}</th><th>{{ $t('legacy.t_84ae58f16b57') }}</th><th>{{ $t('legacy.t_53ebb183604b') }}</th><th>{{ $t('legacy.t_d71d167d09d9') }}</th></tr></thead>
          <tbody>
    ...
    <aside class="panel detail-panel tenancy-detail-panel">
      <div v-if="selectedRow" class="detail-card">
        ...
        <div class="detail-actions tenancy-actions">
          <button :disabled="selectedRow.leaseStatus !== 'active'" @click="openEditLease">{{ $t('legacy.t_f688a08a4a7e') }}</button><button :disabled="selectedRow.leaseStatus !== 'active'" @click="openRenewalLease">{{ $t('processCenter.operationsRenewalButton') }}</button>
          ...
          <button v-if="selectedRow.leaseId" :disabled="contractBusy || templateBusy" @click="openLeaseSigning">{{ selectedRow.contractStatus === 'signed' ? $t('rentalFiles.restartSigning') : $t('legacy.t_403f901d0b71') }}</button>
        ...
        <div v-if="selectedRow.leaseStatus === 'active' && selectedRow.leaseEnd" class="lease-grace-notice" :class="{ active: leaseInGracePeriod }">
          <b>{{ $t(leaseInGracePeriod ? 'tenancy.gracePeriodActive' : 'tenancy.gracePeriodPolicy') }}</b>
          <span>{{ $t(leaseInGracePeriod ? 'tenancy.gracePeriodActiveHint' : 'tenancy.gracePeriodPolicyHint', { date: autoTerminationDate }) }}</span>
        </div>
        ...
        <div class="detail-section"><h4><span class="num">{{ $t('legacy.t_da4b9237bacc') }}</span>{{ $t('legacy.t_1a855f8f0f4f') }}</h4><div class="kv"><span>{{ $t('legacy.t_0c36300c2dae') }}</span><b>{{ selectedRow.rentalSpaceName || $t('legacy.t_2342e6b6433d') }}</b></div><div class="kv"><span>{{ $t('legacy.t_7275c423eae7') }}</span><b>{{ selectedRow.leaseNo || '—' }}</b></div><div class="kv"><span>{{ $t('legacy.t_87d24fa26e57') }}</span><b>{{ selectedRow.leaseStart ? `${selectedRow.leaseStart} ~ ${selectedRow.leaseEnd}` : '—' }}</b></div><div class="kv"><span>{{ $t('legacy.t_4c32ed4c883c') }}</span><b>{{ money(selectedRow.monthlyRent) }}</b></div><div class="kv"><span>{{ $t('legacy.t_986f8c25c20e') }}</span><b>{{ selectedRow.rentCalculationMethod === 'daily_prorated' ? $t('legacy.t_1e5825e68267') : $t('legacy.t_b9c6778f8e79') }}</b></div><div class="kv"><span>{{ $t('legacy.t_52661b02b6a2') }}</span><b>{{ money(selectedRow.depositAmount) }}</b></div><div class="kv"><span>{{ $t('legacy.t_59e1119803f6') }}</span><b>{{ selectedRow.paymentDay ? $t('tenancy.dayValue', { day: selectedRow.paymentDay }) : '—' }}</b></div><div class="kv"><span>{{ $t('legacy.t_56e5d9db491b') }}</span><b>{{ selectedRow.contractDocumentName || $t('legacy.t_3c5b845dce11') }}</b></div><div class="kv"><span>{{ $t('legacy.t_f365bb04b619') }}</span><b>{{ contractStatusLabel(selectedRow.contractStatus) }}</b></div></div>
    ...
    <dialog ref="currentUnpaidDialog" class="modal admin-tenancy-dialog current-unpaid-dialog">
      ...
      <div v-else-if="currentUnpaidError" class="admin-property-error">{{ currentUnpaidError }}</div>
    ...
    <dialog ref="leaseDialog" class="modal admin-tenancy-dialog">
      <form method="dialog" @submit.prevent="submitLease">
        ...
        <div class="form-grid tenancy-form">
          ...
          <label>{{ $t('legacy.t_0c36300c2dae') }}<select v-model.number="leaseForm.rentalSpaceId" required><option :value="null" disabled>{{ $t('legacy.t_be63dfc8d0ab') }}</option><option v-for="space in availableLeaseSpaces" :key="space.id" :value="space.id">{{ space.spaceName }}{{ space.currentLeaseId ? $t('tenancy.occupiedUntil', { date: space.currentLeaseEnd }) : '' }}</option></select><small v-if="selectedLeaseUnit?.rentalMode === 'shared'">{{ $t('legacy.t_e5e7c9d0450e') }}</small></label>
    ...
    <dialog ref="templateDialog" class="modal admin-tenancy-dialog contract-template-dialog">
      <form method="dialog" @submit.prevent="generateTemplate">
        ...
        <div class="form-grid tenancy-form">
          ...
          <label v-if="templateForm.templateType === 'otr'">续租年数<input v-model.trim="templateForm.fields.renewalYears" required></label>
          ...
          <template v-if="templateForm.templateType === 'otr'">
            <label>租客签署日期<input v-model.trim="templateForm.fields.tenantDate" type="date" required></label>
            <label>业主签署日期<input v-model.trim="templateForm.fields.landlordDate" type="date" required></label>
            <label>租客方见证人姓名<input v-model.trim="templateForm.fields.tenantWitnessName" required></label>
            <label>租客方见证人证件号<input v-model.trim="templateForm.fields.tenantWitnessIdentity" required></label>
            <label>租客方见证日期<input v-model.trim="templateForm.fields.tenantWitnessDate" type="date" required></label>
            <label>业主方见证人姓名<input v-model.trim="templateForm.fields.landlordWitnessName" required></label>
            <label>业主方见证人证件号<input v-model.trim="templateForm.fields.landlordWitnessIdentity" required></label>
            <label>业主方见证日期<input v-model.trim="templateForm.fields.landlordWitnessDate" type="date" required></label>
          </template>
    ...
    <dialog ref="leaseSigningDialog" class="modal admin-tenancy-dialog">
      <form method="dialog" @submit.prevent="startLeaseSigning">
        <div class="modal-head"><div><h3>{{ $t('legacy.t_d533a7b75659') }}</h3><small>{{ signatureLinks.length ? $t('rentalFiles.verificationCodeThirtyMinutes') : $t('rentalFiles.signerPackageHint') }}</small></div><button type="button" class="icon-close" @click="closeDialog('leaseSigningDialog')">×</button></div>
        <div v-if="signatureLinks.length" class="tenancy-signing-links">
          <strong>{{ $t('rentalFiles.signingLinksReady') }}</strong>
          <article v-for="link in signatureLinks" :key="link.requestId || link.signingUrl"><span>{{ leaseSignerRoleLabel(link.signerRole) }} · {{ link.signerName }}</span><input :value="link.signingUrl" readonly><button type="button" @click="copyLeaseSigningLink(link.signingUrl)">{{ $t('rentalFiles.copyLink') }}</button><a :href="link.signingUrl" target="_blank" rel="noopener noreferrer">{{ $t('rentalFiles.openSigningLink') }}</a></article>
        </div>
        <div v-else class="form-grid tenancy-form"><section v-for="signer in signatureForm.signers" :key="signer.signerRole"><strong>{{ leaseSignerRoleLabel(signer.signerRole) }}</strong><label>{{ $t('legacy.t_a6c87526ef43') }}<input v-model.trim="signer.signerName" required maxlength="190"></label><label>{{ $t('legacy.t_7659e49d822a') }}<input v-model.trim="signer.signerEmail" required type="email" maxlength="190"></label></section><label>{{ $t('legacy.t_1f156a7776ee') }}<input v-model.number="signatureForm.expiresInDays" required type="number" min="1" max="30"></label><p v-if="actionError" class="admin-property-error">{{ actionError }}</p></div>
    ...
    <dialog ref="renewalLeaseDialog" class="modal admin-tenancy-dialog">
      <form method="dialog" @submit.prevent="submitRenewalLease">
        ...
        <div class="form-grid tenancy-form">
          ...
          <p v-if="renewalError" class="admin-property-error">{{ renewalError }}</p>
        </div>
...
<script>
import { closeAdminLease, createAdminLease, createAdminLeaseRenewal, createAdminTenant, fetchAdminLeaseContract, fetchAdminLeasePeriods, fetchAdminLeaseRenewalContract, fetchAdminLeaseRentInvoices, fetchAdminLeaseSignatureParticipants, fetchAdminPropertyHandover, fetchAdminTenancy, fetchAdminTenancyOptions, generateAdminContractTemplate, sendAdminRentReminder, startAdminLeaseSignaturePackage, transferAdminLease, updateAdminLease, uploadAdminLeaseContract, uploadAdminLeaseRenewalContract } from '../services/propertyApi';
...
export default {
  ...
  methods: {
    ...
    async openLeaseSigningDialog() {
      ...
      this.signatureForm = { signers: defaults, expiresInDays: 7 }; this.signatureLinks = []; this.actionError = '';
    ...
    async startLeaseSigning() { this.signatureBusy = true; this.actionError = ''; try { const result = await startAdminLeaseSignaturePackage(this.selectedRow.leaseId, this.signatureForm); this.signatureLinks = result?.signingLinks || (result?.signingUrl ? [result] : []); this.page.showToast(this.$t('tenancy.signingInvitationSent')); } catch (e) { this.actionError = e.message || this.$t('tenancy.signingInvitationFailed'); } finally { this.signatureBusy = false; } },
    ...
    async sendReminder() { try { await sendAdminRentReminder(this.selectedRow.invoiceId); this.page.showToast(this.$t('tenancy.reminderCreated')); } catch (e) { this.page.showToast(e.message || this.$t('tenancy.reminderCreateFailed')); } },
    ...
    displayDate(value) { return formatDate(value); }, money(v) { return `RM ${Number(v || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; }, initials(name) { return String(name || '').split(/\s+/).slice(0, 2).map(p => p[0]).join('').toUpperCase() || this.$t('ui.tenantIcon'); }
  }
};
</script>

<style scoped>
.lease-change-note,.lease-transfer-warning{grid-column:1/-1;display:grid;gap:5px;padding:11px 13px;border:1px solid #d9e3ed;border-radius:8px;background:#f6f9fc;color:#53667b;font-size:12px}.lease-change-note b{color:#12375f}.lease-transfer-warning{border-color:#efd28b;background:#fff8e7;color:#78570a}.lease-transfer-warning b{color:#8b5d00}.tenancy-actions button:first-child{background:#0b376c;color:#fff;border-color:#0b376c}.tenancy-actions button:disabled{opacity:.5;cursor:not-allowed}.tenancy-actions .danger-action{border-color:#efb4b4;color:#b42318;background:#fff7f7}.lease-grace-notice{display:grid;gap:4px;margin:12px 0;padding:11px 13px;border:1px solid #cfe0ef;border-radius:9px;background:#f4f9fd;color:#42617c;font-size:12px}.lease-grace-notice b{color:#174f77}.lease-grace-notice.active{border-color:#efcf87;background:#fff8e7;color:#75560d}.lease-grace-notice.active b{color:#8a5c00}
...
.tenancy-table-wrap table{min-width:1180px}.tenancy-deposit{color:#0b7f78;white-space:nowrap}
.tenancy-signing-links{display:grid;gap:10px;min-width:min(820px,82vw);padding:16px}.tenancy-signing-links>strong{color:#0b7f78}.tenancy-signing-links article{display:grid;grid-template-columns:minmax(150px,.7fr) minmax(260px,1.5fr) auto auto;align-items:center;gap:8px;padding:10px;border:1px solid #d7e5e8;border-radius:8px}.tenancy-signing-links article input{box-sizing:border-box;width:100%;height:35px;border:1px solid #c9dce2;border-radius:7px;background:#f8fbfc;padding:0 9px}.tenancy-signing-links article button,.tenancy-signing-links article a{display:inline-grid;place-items:center;min-height:35px;border:1px solid #0b8f96;border-radius:7px;background:#fff;padding:0 11px;color:#08747c;text-decoration:none}@media(max-width:800px){.tenancy-signing-links{min-width:0}.tenancy-signing-links article{grid-template-columns:1fr 1fr}.tenancy-signing-links article span,.tenancy-signing-links article input{grid-column:1/-1}}
