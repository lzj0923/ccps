  ...
  <section class="reminder-layout">
    <div class="panel reminder-main">
      <div class="reminder-head">
        ...
        <nav class="reminder-tabs" :aria-label="$t('legacy.t_016c40c5431c')">
          <button type="button" :class="{ active: activeTab === 'collections' }" :aria-pressed="activeTab === 'collections'" @click="activeTab = 'collections'">{{ $t('legacy.t_4febd270ee7d') }} <b>{{ collections.length }}</b></button>
          <button type="button" :class="{ active: activeTab === 'rules' }" :aria-pressed="activeTab === 'rules'" @click="activeTab = 'rules'">{{ $t('ui.reminderRules') }} <b>{{ visibleRules.length }}</b></button>
          <button type="button" :class="{ active: activeTab === 'notifications' }" :aria-pressed="activeTab === 'notifications'" @click="activeTab = 'notifications'">{{ $t('ui.notificationRecords') }} <b>{{ notifications.length }}</b></button>
          <button type="button" :class="{ active: activeTab === 'deliveries' }" :aria-pressed="activeTab === 'deliveries'" @click="activeTab = 'deliveries'">{{ $t('ui.deliveryResults') }} <b>{{ deliveries.length }}</b></button>
        </nav>
      </div>

      <div v-if="loading" class="reminder-state">{{ $t('ui.loadingReminders') }}</div>
      <div v-else-if="errorMessage" class="reminder-state error"><strong>{{ $t('ui.loadRemindersFailed') }}</strong><span>{{ $lt(errorMessage) }}</span><button @click="loadData">{{ $t('ui.reload') }}</button></div>

      <div v-else-if="activeTab === 'collections'" class="table-wrap reminder-table-wrap collection-table-wrap">
        <table>
          <thead><tr><th>{{ $t('legacy.t_7281eef46780') }}</th><th>{{ $t('legacy.t_6ad917dd275c') }}</th><th>{{ $t('legacy.t_2348f9578928') }}</th><th>{{ $t('legacy.t_5eda4c259280') }}</th><th>{{ $t('legacy.t_65a02a484e43') }}</th><th>{{ $t('legacy.t_0a2489f651ae') }}</th><th>{{ $t('legacy.t_7b67aa90f30a') }}</th><th>{{ $t('legacy.t_f3ea6d345e2a') }}</th></tr></thead>
          <tbody>
            <tr v-for="item in pagedCollections" :key="item.invoiceId" :class="{ selected: selectedCollectionInvoiceId === item.invoiceId }" @click="selectedCollectionInvoiceId = item.invoiceId">
              <td><strong>{{ item.tenantName }}</strong><small>{{ item.leaseNo }} · {{ $lt(monthLabel(item.billingMonth)) }}</small><span v-if="item.whatsappEnabled" class="collection-whatsapp">{{ $t('legacy.t_77960639bfa0') }}</span></td>
              <td><strong>{{ item.projectName }}</strong><small>{{ item.unitNo }}</small></td>
              <td>{{ dateTime(item.dueDate) }}</td>
              <td><strong>{{ $t('legacy.t_5e7b60c626a4') }} {{ money(item.outstandingAmount) }}</strong><small>{{ $t('legacy.t_c4dd33a4a832') }} {{ money(item.amountDue) }} {{ $t('legacy.t_239f4237676c') }} {{ money(item.amountPaid) }}</small></td>
              <td><span class="overdue-days">{{ item.overdueDays }} {{ $t('legacy.t_c3304d1e49e3') }}</span></td>
              ...
              <td>{{ displayDate(item.nextActionDate) }}</td>
              <td class="row-buttons"><button v-if="item.currentStage !== 'waiting' && item.currentStage !== 'on_hold'" @click.stop="sendCollectionStage(item)">{{ $lt(collectionActionLabel(item)) }}</button><button v-if="item.workflowStatus === 'on_hold'" @click.stop="resumeCollection(item)">{{ $t('legacy.t_79748ca1c6e5') }}</button><button v-else @click.stop="holdCollection(item)">{{ $t('legacy.t_130448bce675') }}</button></td>
            </tr>
            <tr v-if="!filteredCollections.length"><td colspan="8" class="empty-cell">{{ $t('legacy.t_8cc71779a350') }}</td></tr>
          </tbody>
        </table>
      </div>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="rule in pagedRules" :key="rule.id" :class="{ selected: selectedRuleId === rule.id }" @click="selectedRuleId = rule.id">
              ...
              <td>{{ $lt(eventLabel(rule.eventType)) }}</td>
              <td>{{ $lt(triggerLabel(rule)) }}</td>
              <td><span v-for="channel in rule.channels" :key="channel" class="channel-chip">{{ $lt(channelLabel(channel)) }}</span></td>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="item in pagedNotifications" :key="item.id">
              ...
              <td>{{ item.ruleName }}</td><td><span class="reminder-tag" :class="priorityClass(item.priority)">{{ $lt(priorityLabel(item.priority)) }}</span></td>
              <td>{{ $lt(deliveryLabel(item.deliverySummary)) }}</td><td class="failure-cell">{{ item.failureReason || '—' }}</td>
            </tr>
        ...
        <table>
          ...
          <tbody>
            <tr v-for="delivery in pagedDeliveries" :key="delivery.id">
              ...
              <td>{{ $lt(channelLabel(delivery.channel)) }}</td><td>{{ delivery.destination || $t('legacy.t_1be94bed7b3f') }}</td>
              <td><span class="reminder-tag" :class="delivery.status">{{ $lt(deliveryStatusLabel(delivery.status)) }}</span></td><td>{{ delivery.attemptCount }}</td>
    ...
    <aside class="panel reminder-detail">
      <template v-if="activeTab === 'collections' && selectedCollection">
        <div class="rule-title"><div class="rule-icon collection-icon">{{ $t('legacy.t_1cf84065310b') }}</div><div><h3>{{ selectedCollection.tenantName }}</h3><p>{{ selectedCollection.projectName }} · {{ selectedCollection.unitNo }}</p></div><span class="reminder-tag" :class="collectionStageClass(selectedCollection)">{{ selectedCollection.currentStageLabel }}</span></div>
        <section class="collection-summary"><div><span>{{ $t('legacy.t_296a9b9bfd5e') }}</span><strong>{{ $t('legacy.t_5e7b60c626a4') }} {{ money(selectedCollection.outstandingAmount) }}</strong></div><div><span>{{ $t('legacy.t_bf6fc7bf226d') }}</span><strong>{{ selectedCollection.overdueDays }} {{ $t('legacy.t_c3304d1e49e3') }}</strong></div></section>
        <section class="rule-section"><h4>{{ $t('legacy.t_dc82a57f2aad') }}</h4><ol class="collection-timeline"><li v-for="stage in collectionStages" :key="stage.code" :class="{ done: selectedCollection.sentStages.includes(stage.code), current: selectedCollection.currentStage === stage.code }"><i></i><div><b>{{ $lt(stage.label) }}</b><small>{{ $t('legacy.t_65a02a484e43') }} {{ stage.days }} {{ $t('legacy.t_dd801db158f2') }} {{ datePlus(selectedCollection.dueDate, stage.days) }}</small></div></li></ol></section>
<section class="rule-section"><h4>{{ $t('legacy.t_5505df382809') }}</h4><dl><div><dt>{{ $t('legacy.t_f39c6b629f80') }}</dt><dd>{{ selectedCollection.tenantPhone || $t('legacy.t_287a1d10348b') }}</dd></div><div><dt>{{ $t('legacy.t_9ed627bcf63d') }}</dt><dd>{{ selectedCollection.tenantEmail || $t('legacy.t_287a1d10348b') }}</dd></div><div><dt>{{ $t('legacy.t_b336fc558722') }}</dt><dd :class="{ 'whatsapp-ready': selectedCollection.whatsappEnabled }">{{ selectedCollection.whatsappEnabled ? $t('ui.whatsappAuthorizedTo', { destination: selectedCollection.whatsappDestination }) : $t('legacy.t_ba9e7b143ecb') }}</dd></div><div><dt>{{ $t('legacy.t_e664b632f282') }}</dt><dd>{{ displayDate(selectedCollection.dueDate) }}</dd></div></dl></section>
        <section v-if="selectedCollection.currentStage === 'termination_notice'" class="termination-warning"><strong>{{ $t('legacy.t_a23f49bd4f22') }}</strong><p>{{ $t('legacy.t_95aead1218db') }}</p></section>
        <div class="detail-actions collection-actions"><button v-if="selectedCollection.currentStage !== 'waiting' && selectedCollection.currentStage !== 'on_hold'" @click="sendCollectionStage(selectedCollection)">{{ $lt(collectionActionLabel(selectedCollection)) }}</button><button v-if="selectedCollection.workflowStatus === 'on_hold'" @click="resumeCollection(selectedCollection)">{{ $t('legacy.t_54255238b875') }}</button><button v-else @click="holdCollection(selectedCollection)">{{ $t('legacy.t_39432fc358d3') }}</button></div>
      </template>
      <template v-else-if="activeTab === 'rules' && selectedRule">
        ...
        <section class="rule-section"><h4>{{ $t('legacy.t_a467b6df2ddd') }}</h4><dl><div><dt>{{ $t('legacy.t_fd233f798e10') }}</dt><dd>{{ $lt(eventLabel(selectedRule.eventType)) }}</dd></div><div><dt>{{ $t('legacy.t_b0f7fc5e2c79') }}</dt><dd>{{ $lt(triggerLabel(selectedRule)) }}</dd></div><div><dt>{{ $t('legacy.t_bc14276958f3') }}</dt><dd>{{ $t('legacy.t_ad6da4dc7810') }}</dd></div><div><dt>{{ $t('legacy.t_836eeef0f9bd') }}</dt><dd>{{ selectedRule.channels.map(channelLabel).join('、') }}</dd></div></dl></section>
      ...
      <div v-else class="reminder-state">{{ activeTab === 'collections' ? $t('legacy.t_9a418eacb994') : $t('legacy.t_60ee16d27951') }}</div>
    </aside>
    ...
    <dialog ref="ruleDialog" class="rule-dialog">
      <form @submit.prevent="saveRule">
        ...
        <div class="rule-form">
          ...
          <div class="form-grid"><label>{{ $t('legacy.t_fd233f798e10') }}<select v-model="form.eventType"><option v-for="option in eventOptions" :key="option.value" :value="option.value">{{ $lt(option.label) }}</option></select></label><label>{{ $t('legacy.t_1c53bb9b196e') }}<input v-model.number="form.daysBefore" type="number" min="0" max="365" required><small v-if="form.eventType === 'rent_due'">{{ $t('legacy.t_d93e4932d378') }}</small></label></div>
          <fieldset><legend>{{ $t('legacy.t_836eeef0f9bd') }}</legend><label v-for="channel in channelOptions" :key="channel.value" class="check-label"><input v-model="form.channels" type="checkbox" :value="channel.value" :disabled="channel.value === 'whatsapp' && !['rent_due', 'lease_expiry'].includes(form.eventType)"><span><b>{{ $lt(channel.label) }}</b><small>{{ $lt(channel.hint) }}</small></span></label></fieldset>
          <p v-if="form.eventType === 'rent_due' && form.channels.includes('whatsapp')" class="whatsapp-rule-note">{{ $t('legacy.t_30790ff719fe') }}</p>
          <p v-if="form.eventType === 'lease_expiry' && form.channels.includes('whatsapp')" class="whatsapp-rule-note">{{ $t('legacy.t_076ac53b7817') }}</p>
...
export default {
  ...
  computed: {
    visibleRules() { return this.rules.filter(rule => !rule.systemManaged); },
    selectedRule() { return this.visibleRules.find(rule => rule.id === this.selectedRuleId) || this.filteredRules[0] || null; },
    ...
    filteredRules() { const status = this.page.statusFilter; return this.visibleRules.filter(rule => (!this.keyword || `${rule.name} ${rule.code} ${this.eventLabel(rule.eventType)}`.toLowerCase().includes(this.keyword)) && (!status || status.includes('全部') || (status === '啟用' && rule.enabled) || (status === '停用' && !rule.enabled))); },
    ...
    createNonce() { return this.page.adminReminderCreateNonce; }, runNonce() { return this.page.adminReminderRunNonce; }, openNotificationsNonce() { return this.page.adminReminderOpenNotificationsNonce; }
  },
  watch: {
    createNonce(value, previous) { if (value > previous) this.openCreate(); },
    runNonce(value, previous) { if (value > previous) this.runAll(); },
    openNotificationsNonce(value, previous) { if (value > previous) this.openNotifications(); },
  ...
  mounted() { this.page.projectFilter = '全部提醒'; this.page.statusFilter = '全部狀態'; if (this.openNotificationsNonce > 0) this.openNotifications(); this.loadData(); },
  methods: {
    openNotifications() { this.activeTab = 'notifications'; this.pageNumber = 1; },
    ...
    async loadData() {
      ...
      try {
        ...
        if (!this.visibleRules.some(rule => rule.id === this.selectedRuleId)) this.selectedRuleId = this.visibleRules[0]?.id || null;
    ...
    setMetrics(summary) {
      this.page.adminReminderMetrics = [
        { label: this.$t('ui.reminderRules'), value: this.$t('ui.records', { count: this.visibleRules.length }), delta: this.$t('ui.enabledRules', { count: this.visibleRules.filter(rule => rule.enabled).length }), trend: 'up' },
    ...
    async deleteRule(rule) { if (rule.systemManaged || !window.confirm(this.$ltf`確定刪除「${rule.name}」？`)) return; try { await deleteAdminReminderRule(rule.id); await this.loadData(); this.page.showToast('提醒規則已刪除'); } catch (error) { this.page.showToast(error.message || '提醒規則刪除失敗'); } },
    async runRule(rule) { if (rule.systemManaged) return; this.runningId = rule.id; try { const result = await runAdminReminderRule(rule.id); await this.loadData(); if (rule.channels.includes('whatsapp')) { this.activeTab = 'deliveries'; this.page.statusFilter = '全部狀態'; this.refreshWhatsAppResult(); } this.page.showToast(this.$ltf`已建立 ${Number(result.createdCount || 0)} 則通知${rule.channels.includes('whatsapp') ? '，WhatsApp 投递结果已显示' : ''}`); } catch (error) { this.page.showToast(error.message || '提醒規則執行失敗'); } finally { this.runningId = null; } },
    async runAll() { try { const result = await runAllAdminReminderRules(); await this.loadData(); this.page.showToast(this.$ltf`全部規則執行完成，建立 ${Number(result.createdCount || 0)} 則通知`); } catch (error) { this.page.showToast(error.message || '批量執行失敗'); } },
    async retryDelivery(delivery) { try { await retryAdminReminderDelivery(delivery.id); await this.loadData(); this.page.showToast(this.$ltf`${this.channelLabel(delivery.channel)}已加入重试队列`); } catch (error) { this.page.showToast(error.message || '重试失败'); } },
    async sendCollectionStage(item) { const termination = item.currentStage === 'termination_notice'; const prompt = termination ? this.$lt('确认向租客发送终止通知？发送后仍需人工办理解约及停卡。') : this.$ltf`确认发送“${item.currentStageLabel}”？该账单尚欠 RM ${this.money(item.outstandingAmount)}，已逾期 ${item.overdueDays} 天。`; if (!window.confirm(prompt)) return; try { await sendAdminRentCollectionStage(item.invoiceId, item.currentStage); await this.loadData(); this.activeTab = 'deliveries'; this.page.statusFilter = '全部狀態'; this.refreshWhatsAppResult(); this.page.showToast(this.$ltf`${item.currentStageLabel}已建立，正在发送 WhatsApp`); } catch (error) { this.page.showToast(error.message || '催缴通知发送失败'); } },
    refreshWhatsAppResult() { window.setTimeout(() => this.loadData(), 4000); window.setTimeout(() => this.loadData(), 17000); },
    async holdCollection(item) { const reason = window.prompt(this.$lt('请输入暂停催缴原因，例如租金争议、付款核对中：')); if (!reason?.trim()) return; try { await holdAdminRentCollection(item.invoiceId, reason.trim()); await this.loadData(); this.page.showToast('该笔催缴已暂停'); } catch (error) { this.page.showToast(error.message || '暂停催缴失败'); } },
    async resumeCollection(item) { if (!window.confirm(this.$lt('确认恢复该笔租金催缴？'))) return; try { await resumeAdminRentCollection(item.invoiceId); await this.loadData(); this.page.showToast('催缴已恢复'); } catch (error) { this.page.showToast(error.message || '恢复催缴失败'); } },
    ...
    datePlus(value, days) { if (!value) return '—'; const date = new Date(`${value}T00:00:00`); date.setDate(date.getDate() + days); return formatDate(date.toISOString().slice(0, 10)); },
...
<style scoped>
/* Hallmark · component: tab strip · genre: modern-minimal · theme: CCPS workbench
...
.form-grid small{color:#718096;font-size:11px;font-weight:400}.check-label input:disabled+span{opacity:.5}.whatsapp-rule-note{margin:0;padding:10px 12px;border:1px solid #bde6cb;border-radius:8px;background:#eef9f2;color:#176c40;font-size:12px;line-height:1.55}
</style>
