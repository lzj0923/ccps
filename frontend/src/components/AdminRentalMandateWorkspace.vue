<template>
  <section class="mandate-workspace" :class="{ 'workflow-pending': workflowPending }">
    <div class="workspace-head">
      ...
      <button class="primary-btn primary-btn-lg" type="button" @click="showCreate = true"><Plus :size="18" :stroke-width="2.2" />{{ $t('legacy.t_42bf6a51df77') }}</button>
    </div>
    <div class="summary-grid"><div v-for="card in summaryCards" :key="card.key" class="summary-card" :class="`summary-${card.key}`"><span class="summary-icon"><component :is="card.icon" :size="21" :stroke-width="1.9" /></span><div class="summary-copy"><span>{{ card.label }}</span><strong>{{ summary[card.key] || 0 }}</strong><small>{{ $t('legacy.t_01992f531941') }}</small></div></div></div>
    ...
    <aside v-if="selected" class="detail-panel"><div class="detail-head"><div><div class="eyebrow">{{ $t('legacy.t_1acb9dd33ebd') }}</div><h3>{{ selected.mandateNo }}</h3></div><button class="icon-btn" type="button" :aria-label="$t('legacy.t_48b47156015e')" @click="selected = null">×</button></div><div class="detail-hero"><strong>{{ selected.ownerName }}</strong><span>{{ selected.projectName }} · {{ selected.unitNo }}</span><span class="status" :class="selected.status"><i></i>{{ statusLabel(selected.status) }}</span></div><div class="detail-grid"><div><small>{{ $t('legacy.t_0e996dca5673') }}</small><strong>{{ selected.startDate }} {{ $t('legacy.t_43401e739ef4') }} {{ selected.endDate || $t('legacy.t_79aacc634aa7') }}</strong></div><div><small>{{ $t('legacy.t_a178daac2527') }}</small><strong>{{ selected.managementFee || 0 }}</strong></div><div><small>{{ $t('legacy.t_93a98f5e6002') }}</small><strong>{{ selected.commissionPercent || 0 }}%</strong></div></div><div><div v-if="['active','suspended'].includes(selected.status)" class="detail-actions"><button class="secondary-btn full-btn" type="button" @click="openHandover"><span>⌂</span>{{ handover ? $t('legacy.t_3d082ac64340') : $t('legacy.t_7311f7b91045') }}</button></div><p v-if="handover" class="handover-state"><i></i>{{ $t('legacy.t_494a40854fe7') }}{{ handover.status === 'completed' ? $t('legacy.t_e99b48a29bdf') : $t('legacy.t_0f436818c0b4') }} · {{ handover.handoverDate }}</p><section class="detail-section"><div class="section-title"><h4>{{ $t('legacy.t_2cb33d0a43c5') }}</h4><span>{{ documentsLoading ? $t('tenancy.loadingDocuments') : documents.length }} {{ $t('legacy.t_aa9f1ad4f91c') }}</span></div><p class="authorization-hint">{{ $t('tenancy.signedAuthorizationUploadHint') }}</p><div class="upload-row"><select v-model="documentType"><option value="mandate_document">{{ $t('legacy.t_01cbac7e5bc6') }}</option><option value="authorization">{{ $t('legacy.t_5ed1fcbc5cd9') }}</option><option value="handover_photo">{{ $t('legacy.t_91594df5910f') }}</option><option value="inventory">{{ $t('legacy.t_c8159b903b04') }}</option></select><label class="upload-btn"><span>↑</span>{{ $t('legacy.t_cc44896ba419') }}<input type="file" @change="uploadDocument" /></label></div><p v-if="documentsLoading" class="muted">{{ $t('tenancy.loadingDocuments') }}</p><p v-else-if="documentLoadError" class="admin-property-error">{{ documentLoadError }}</p><ul v-else class="detail-list"><li v-for="doc in documents" :key="doc.id"><div><button class="link-btn" type="button" @click="downloadDocument(doc)">{{ doc.originalName }}</button><small>{{ doc.relationType }} · {{ doc.createdAt }}</small></div><button v-if="doc.mimeType === 'application/pdf' && doc.relationType !== 'signed_contract'" class="text-action" type="button" @click="startDocumentSigning(doc)">{{ $t('legacy.t_403f901d0b71') }}</button></li><li v-if="!documents.length" class="muted">{{ $t('legacy.t_6007668430ab') }}</li></ul></section><section class="detail-section"><div class="section-title"><h4>{{ $t('legacy.t_04dcf3bd904a') }}</h4><span>{{ history.length }} {{ $t('legacy.t_d0bb9b2b8ea7') }}</span></div><ul class="history-list"><li v-for="entry in history" :key="entry.id"><span class="history-dot"></span><div><strong>{{ entry.fromStatus || '—' }} → {{ entry.toStatus }}</strong><small>{{ entry.changedAt }}<template v-if="entry.reason"> · {{ entry.reason }}</template></small></div></li></ul></section></div></aside>

    <div v-if="authorizationWorkspaceOpen" class="modal-backdrop" @click.self="closeAuthorizationWorkspace"><div class="modal modal-authorization-workspace"><div class="modal-head"><div><div class="eyebrow">{{ $t('tenancy.authorizationWorkspaceEyebrow') }}</div><h3>{{ $t('tenancy.authorizationWorkspaceTitle') }}</h3><p>{{ selected && selected.mandateNo }} · {{ selected && selected.ownerName }}</p></div><button type="button" class="icon-btn" @click="closeAuthorizationWorkspace">×</button></div><div class="authorization-steps"><span class="active">1. {{ $t('tenancy.authorizationStepGenerate') }}</span><span :class="{ active: authorizationDraft }">2. {{ $t('tenancy.authorizationStepSign') }}</span><span :class="{ active: hasAuthorization }">3. {{ $t('tenancy.authorizationStepComplete') }}</span></div><p v-if="!authorizationDraft && !hasAuthorization" class="authorization-hint">{{ $t('tenancy.authorizationWorkspaceHint') }}</p><button v-if="!authorizationDraft && !hasAuthorization" class="primary-btn" type="button" @click="openAuthorizationForm">{{ $t('tenancy.generateAuthorization') }}</button><div v-if="authorizationDraft" class="authorization-document-row"><div class="authorization-document-info"><strong>{{ authorizationDraft.originalName }}</strong><small>{{ $t('tenancy.authorizationDraftLabel') }}</small></div><div class="signing-recipient-fields"><label>{{ $t('legacy.t_a6c87526ef43') }}<input v-model.trim="signingForm.signerName" type="text" autocomplete="name"></label><label>{{ $t('legacy.t_7659e49d822a') }}<input v-model.trim="signingForm.signerEmail" type="email" autocomplete="email" required></label></div><button class="primary-btn compact-btn" type="button" @click="startDocumentSigning(authorizationDraft)" :disabled="signingBusy || !signingForm.signerName.trim() || !signingForm.signerEmail.trim()">{{ signingBusy ? '发送中…' : $t('tenancy.startOnlineSigning') }}</button></div><p v-if="signingError" class="admin-property-error">{{ signingError }}</p><p v-if="hasAuthorization" class="authorization-ready">{{ $t('tenancy.authorizationUploaded') }}</p><div class="modal-actions"><button class="secondary-btn" type="button" @click="closeAuthorizationWorkspace">{{ $t('tenancy.close') }}</button></div></div></div>
    <div v-if="authorizationOpen" class="modal-backdrop" @click.self="closeAuthorizationDialog"><div class="modal mandate-authorization-dialog"><form @submit.prevent="generateAuthorization"><div class="modal-head"><div><h3>{{ $t('tenancy.generateAuthorizationTitle') }}</h3><small>{{ $t('tenancy.templateHint') }}</small></div><button type="button" class="icon-btn" @click="closeAuthorizationDialog">×</button></div><div class="form-grid"><label>{{ $t('tenancy.caseNo') }}<input v-model.trim="authorizationForm.caseNo" required></label><label>{{ $t('tenancy.propertyAddress') }}<input v-model.trim="authorizationForm.propertyAddress" required></label><label>{{ $t('tenancy.ownerName') }}<input v-model.trim="authorizationForm.landlordName"></label><label>{{ $t('tenancy.ownerIdentity') }}<input v-model.trim="authorizationForm.landlordIdentity"></label><label>{{ $t('tenancy.earnestDepositRm') }}<input v-model.trim="authorizationForm.earnestDeposit"></label><label>{{ $t('tenancy.commissionMonths') }}<input v-model.trim="authorizationForm.commission"></label><label>{{ $t('tenancy.gstSst') }}<input v-model.trim="authorizationForm.securityDepositMonths"></label><label>{{ $t('tenancy.agencyFeeRm') }}<input v-model.trim="authorizationForm.utilityDepositMonths"></label><label>{{ $t('tenancy.totalAgencyFeeRm') }}<input v-model.trim="authorizationForm.totalBeforeKeys"></label><label>{{ $t('tenancy.mandateStartDate') }}<input v-model.trim="authorizationForm.startDate" type="date"></label><label>{{ $t('tenancy.mandateEndDate') }}<input v-model.trim="authorizationForm.commencementDate" type="date"></label><label class="wide">{{ $t('tenancy.otherConditions') }}<input v-model.trim="authorizationForm.otherConditions"></label></div><p v-if="authorizationError" class="admin-property-error">{{ authorizationError }}</p><div class="modal-actions"><button class="secondary-btn" type="button" @click="closeAuthorizationDialog">{{ $t('tenancy.cancel') }}</button><button class="primary-btn" type="submit" :disabled="authorizationBusy">{{ authorizationBusy ? $t('tenancy.generating') : $t('tenancy.generateDownloadPdf') }}</button></div></form></div></div>
    <div v-if="showCreate" class="modal-backdrop"><div class="modal"><div class="modal-head"><div><div class="eyebrow">{{ $t('legacy.t_1d6e5efa9017') }}</div><h3>{{ $t('legacy.t_fd996f9aad00') }}</h3><p>{{ $t('legacy.t_5eadd0312f61') }}</p></div><button class="icon-btn" type="button" @click="showCreate = false">×</button></div><label>{{ $t('legacy.t_805febdfe9ae') }}<select v-model="form.ownerUnitId"><option :value="null">{{ $t('legacy.t_9d72e0df80ca') }}</option><option v-for="unit in options.units" :key="unit.id" :value="unit.id">{{ unit.projectName }} · {{ unit.unitNo }} · {{ unit.ownerName }}</option></select></label><label>{{ $t('legacy.t_ca209c5c82b3') }}<select v-model="form.mandateType"><option value="management">{{ $t('legacy.t_884e447b954b') }}</option><option value="exclusive">{{ $t('legacy.t_5e9343efd79f') }}</option><option value="non_exclusive">{{ $t('legacy.t_7da9307fbbf8') }}</option></select></label><div class="form-grid"><label>{{ $t('legacy.t_7fd7a227e9ce') }}<input v-model="form.startDate" type="date" /></label><label>{{ $t('legacy.t_27eefa5237a0') }}<input v-model="form.endDate" type="date" /></label><label>{{ $t('legacy.t_a178daac2527') }}<input v-model="form.managementFee" type="number" min="0" step="0.01" /></label><label>{{ $t('legacy.t_93a98f5e6002') }}<input v-model="form.commissionPercent" type="number" min="0" max="100" step="0.01" /></label></div><label>{{ $t('legacy.t_edcfdcdbfa8c') }}<select v-model="form.responsibleUserId"><option :value="null">{{ $t('legacy.t_863a3c74cfd9') }}</option><option v-for="user in options.users" :key="user.id" :value="user.id">{{ user.displayName }}</option></select></label><div class="modal-actions"><button class="secondary-btn" type="button" @click="showCreate = false">{{ $t('legacy.t_4d0b4688c787') }}</button><button class="primary-btn" type="button" @click="create">{{ $t('legacy.t_cfd822039051') }}</button></div></div></div>
...
<script>
import { fetchAdminRentalMandates, fetchAdminRentalMandateOptions, createAdminRentalMandate, updateAdminRentalMandateStatus, fetchAdminRentalMandateHistory, fetchAdminPropertyHandover, saveAdminPropertyHandover, fetchAdminRentalMandateDocuments, uploadAdminRentalMandateDocument, downloadAdminRentalMandateDocument, startAdminMandateDocumentSignature, generateAdminContractTemplate } from '../services/propertyApi';
...
export default {
  ...
  methods: {
    async applyProcessRoute() { const params = new URLSearchParams(window.location.search); const workflow = params.get('workflow'); const ownerUnitId = params.get('ownerUnitId'); if (!workflow || !ownerUnitId) return; if (workflow === 'create') { await this.loadOptions(); this.form = { ...this.form, ownerUnitId: Number(ownerUnitId) }; this.showCreate = true; return; } const data = await fetchAdminRentalMandates({ page: 1, pageSize: 200 }); const item = (data.rows || data.items || []).find(row => String(row.ownerUnitId) === String(ownerUnitId)); if (!item) return; if (workflow === 'select') { await this.select(item); return; } await this.select(item); this.openAuthorizationWorkspace(item); },
    ...
    async openCreate() { await this.loadOptions(); this.showCreate = true; },
    async create() { try { await createAdminRentalMandate({ ...this.form, ownerUnitId: Number(this.form.ownerUnitId), responsibleUserId: this.form.responsibleUserId ? Number(this.form.responsibleUserId) : null }); this.showCreate = false; this.$emit('toast', '委托已建立并启用'); await Promise.all([this.load(), this.loadOptions()]); } catch (error) { this.$emit('toast', error.message); } },
    ...
    async terminate(item) { const reason = window.prompt('請輸入終止原因'); if (!reason) return; try { await updateAdminRentalMandateStatus(item.id, { status: 'terminated', reason }); await Promise.all([this.load(), this.loadOptions()]); this.$emit('toast', '委託已終止，房產已重新開放建立委託'); } catch (error) { this.$emit('toast', error.message); } },
    ...
    statusLabel(status) { return status === 'expired' ? this.$t('tenancy.expiredStatus') : ({ active: this.$t('legacy.t_ce6c3dc32674'), suspended: this.$t('legacy.t_37676bdf08e6'), terminated: this.$t('legacy.t_7ba87aca8368') }[status] || status); }
  }
};
</script>
