<template>
  <section class="reserve-dashboard-page">
    <div class="reserve-summary-grid">
      <article v-for="card in summaryCards" :key="card.label" class="reserve-summary-card">
        ...
        <div class="reserve-summary-copy">
          <span>{{ card.label }} <i v-if="card.info">i</i></span>
    ...
    <div class="reserve-layout">
      <main class="reserve-main-column">
        <section class="reserve-warning-banner" :class="{ sufficient: !summary.lowBalanceCount }">
          ...
          <div>
            <h2>{{ summary.lowBalanceCount ? '部分預備金餘額低於最低標準' : '目前預備金餘額充足' }}</h2>
            <p v-if="summary.lowBalanceCount">共有 {{ summary.lowBalanceCount }} 個單位需要充值；當前總餘額 RM {{ money(summary.totalBalance) }}，最低標準合計 RM {{ money(summary.minimumBalance) }}。</p>
            <p v-else>您名下 {{ summary.accountCount || 0 }} 個預備金賬戶均不低於最低標準；當前總餘額 RM {{ money(summary.totalBalance) }}。</p>
          </div>
          <button type="button" @click="openTopup"><span v-html="icons.wallet"></span>立即充值</button>
        </section>
        ...
        <section class="reserve-account-strip">
          <article v-for="account in accounts" :key="account.id" :class="{ low: account.balanceStatus === 'low' }">
            ...
            <div><span>目前餘額</span><b>RM {{ money(account.currentBalance) }}</b></div>
            <div><span>最低標準</span><b>RM {{ money(account.minimumBalance) }}</b></div>
            <em>{{ account.balanceStatus === 'low' ? '不足' : '充足' }}</em>
          </article>
        </section>
        ...
        <section class="reserve-table-card">
          <header class="reserve-table-toolbar">
            <h2>預備金交易明細</h2>
            <div>
              <select v-model="draftFilters.projectId" aria-label="房產"><option value="">全部房產</option><option v-for="property in properties" :key="property.id" :value="String(property.id)">{{ property.name }}</option></select>
              <div class="reserve-date-fields"><input v-model="draftFilters.startDate" type="date" aria-label="交易開始日期"><span>至</span><input v-model="draftFilters.endDate" type="date" aria-label="交易結束日期"></div>
              <select v-model="draftFilters.type" aria-label="交易類型"><option value="">全部類型</option><option value="topup">充值</option><option value="debit">扣款</option><option value="adjustment">人工調整</option></select>
              <button type="button" :disabled="loading" @click="applyFilters">{{ loading ? '查詢中' : '查詢' }}</button>
              <button type="button" @click="exportDetailsCsv"><span v-html="icons.download"></span>匯出明細</button>
            </div>
          </header>
          <div class="reserve-table-wrap">
            <table class="reserve-transaction-table">
              <thead><tr><th>日期</th><th>房產 / 類型</th><th>說明</th><th>變動金額</th><th>餘額</th><th>狀態</th><th>財務確認</th></tr></thead>
              <tbody>
                <tr v-for="record in pagedRecords" :key="`${record.id}-${record.status}`">
                  ...
                  <td :class="direction(record.transactionType) === 'in' ? 'reserve-money-in' : 'reserve-money-out'">{{ direction(record.transactionType) === 'in' ? '+' : '-' }} RM {{ money(record.amount) }}</td>
                  ...
                  <td><strong>{{ record.confirmationStatus === 'confirmed' ? formatDateTime(record.occurredAt) : statusLabel(record.confirmationStatus) }}</strong><small v-if="record.attachmentCount">含 {{ record.attachmentCount }} 個憑證</small><small v-else>{{ record.confirmationStatus === 'confirmed' ? 'CCPS 財務部' : '等待財務處理' }}</small></td>
                </tr>
                <tr v-if="loading"><td colspan="7" class="reserve-empty">正在讀取數據…</td></tr>
                <tr v-else-if="!pagedRecords.length"><td colspan="7" class="reserve-empty">沒有符合條件的交易記錄</td></tr>
              </tbody>
            </table>
          </div>
          <footer class="reserve-table-footer">
            <span>共 {{ transactions.length }} 條記錄</span>
      ...
      <aside class="reserve-side-panel">
        <div class="reserve-side-tabs"><button :class="{ active: sideTab === 'notice' }" @click="sideTab = 'notice'">通知中心</button><button :class="{ active: sideTab === 'documents' }" @click="sideTab = 'documents'">充值憑證</button></div>
        <section v-if="sideTab === 'notice'" class="reserve-notice-section">
          <header><h2>最新通知</h2><span>共 {{ notifications.length }} 則</span></header>
          <div class="reserve-notice-list">
            ...
            <p v-if="!notifications.length" class="reserve-side-empty">暫無通知</p>
          </div>
        </section>
        <section v-if="sideTab === 'documents'" class="reserve-document-section standalone">
          <header><h2>充值憑證</h2><span>共 {{ documents.length }} 份</span></header>
          <div class="reserve-document-list">
            <article v-for="document in documents" :key="document.id">
              ...
              <button type="button" @click="viewDocument(document)">預覽</button><button type="button" aria-label="下載文件" @click="downloadDocument(document)" v-html="icons.download"></button>
            </article>
            <p v-if="!documents.length" class="reserve-side-empty">充值後，憑證會顯示在這裡</p>
          </div>
        </section>
      </aside>
    </div>
    ...
    <div v-if="topupVisible" class="reserve-topup-overlay" @click.self="closeTopup">
      <section class="reserve-topup-modal" role="dialog" aria-modal="true" aria-label="提交預備金充值">
        <header><div><h2>預備金充值申請</h2><p>提交後由財務審核，審核通過才會增加餘額。</p></div><button type="button" aria-label="關閉" @click="closeTopup">×</button></header>
        <form @submit.prevent="submitTopup">
          <div class="reserve-topup-form">
            <label class="wide"><span>充值賬戶</span><select v-model="topupForm.reserveAccountId" required><option value="" disabled>請選擇單位</option><option v-for="account in accounts" :key="account.id" :value="String(account.id)">{{ account.projectName }} · {{ account.unitNo }}（RM {{ money(account.currentBalance) }}）</option></select></label>
            <label><span>充值金額 (RM)</span><input v-model="topupForm.amount" type="number" min="0.01" max="1000000" step="0.01" required></label>
            <label><span>付款日期</span><input v-model="topupForm.paymentDate" type="date" :max="today" required></label>
            <label><span>付款方式</span><select v-model="topupForm.paymentMethod" required><option value="bank_transfer">銀行轉賬</option><option value="online_payment">線上支付</option></select></label>
            <label><span>付款人姓名</span><input v-model.trim="topupForm.payerName" maxlength="160" required></label>
            <label><span>銀行 / 支付平台</span><input v-model.trim="topupForm.bankName" maxlength="60" required></label>
            <label><span>交易參考號</span><input v-model.trim="topupForm.reference" maxlength="60" required></label>
            <label class="wide"><span>備註</span><textarea v-model.trim="topupForm.note" maxlength="200" rows="2"></textarea></label>
            <label class="wide reserve-topup-files"><span>付款憑證（JPG、PNG、PDF；最多 4 個）</span><input type="file" accept="image/jpeg,image/png,application/pdf" multiple required @change="selectTopupFiles"><small v-if="topupFiles.length">已選擇 {{ topupFiles.length }} 個文件：{{ topupFiles.map(file => file.name).join('、') }}</small></label>
          </div>
          <p v-if="topupError" class="reserve-topup-error">{{ topupError }}</p>
          <footer><button type="button" @click="closeTopup">取消</button><button class="primary" type="submit" :disabled="submitting">{{ submitting ? '提交中…' : '提交充值申請' }}</button></footer>
        </form>
      </section>
    </div>
  </section>
