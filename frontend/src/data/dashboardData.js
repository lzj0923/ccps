...
const modules = [
  ...
  },
  {
    ...
    title: "出租委託管理", category: "ADMIN RENTAL MANDATE", hero: "建立出租委託、送審、核准並啟用租管服務。",
    hint: "R02 第一階段：委託狀態與審核歷史。", table: "出租委託列表", searchHint: "搜尋委託編號 / 業主 / 單位",
  ...
  {
    id: "adminSync", shell: "admin-shell", code: "08", name: "SQL Account同步", avatar: false,
    title: "SQL Account 同步", category: "ADMIN SYNC", hero: "同步業主資料、租金帳單、房款收款、維修支出與付款憑證。",
    hint: "補齊設計稿和思維導圖中的會計系統串接。",
    table: "SQL Account 同步紀錄", searchHint: "搜尋批次號 / 同步內容",
    primaryAction: "立即同步", secondaryAction: "匯入批次",
    flowTitle: "會計同步流程", flowHint: "未同步、同步中、已同步、同步失敗和失敗原因清楚顯示。",
    quickActions: ["立即同步", "查看差異", "重試失敗", "下載日誌"],
    detailBlocks: detailBlocks(
      { title: "同步內容", items: [["客戶資料", "已完成"], ["租金帳單", "正常"], ["維修支出", "待處理"]] },
      { title: "同步狀態", items: [["未同步", "待處理"], ["同步失敗", "待確認"], ["錯誤日誌", "已完成"]] }
    )
  },
  {
...
const featureMap = {
  ...
  adminSync: [["N1", "同步內容", ["業主及客戶資料", "租金帳單", "房款收款記錄"]], ["N2", "狀態顯示", ["未同步", "同步中", "已同步", "同步失敗"]], ["N3", "錯誤處理", ["失敗原因", "重試批次", "同步日誌"]]]
};
...
const workflowMap = {
  ...
  adminSync: ["選擇資料", "送出同步", "查看狀態", "處理失敗", "保存日誌"]
};
...
const rows = {
  ...
  adminSync: [["SQL-20250522-01", "業主及客戶資料", "128", "126", "2", "待處理", "電話格式缺失", "2025-05-22 09:08"], ["SQL-20250521-02", "租金帳單", "86", "86", "0", "已完成", "-", "2025-05-21 18:04"], ["SQL-20250520-01", "房款收款記錄", "39", "37", "2", "待確認", "憑證缺失", "2025-05-20 18:06"]]
};
...
const headers = {
  ...
  adminSync: ["批次號", "同步內容", "筆數", "成功", "失敗", "同步狀態", "失敗原因", "完成時間", "操作"]
};
...
Object.assign(adminKpis, {
  adminSync: [
    ["鏈", "SQL 連線狀態", "已連線", "延遲 42ms", "up"],
    ["同", "同步成功率", "98.6%", "↑ 1.2%", "up"],
    ["批", "今日批次", "18 批", "6 批待處理", "down"],
    ["成", "成功筆數", "1,286 筆", "本日累計", "up"],
    ["敗", "失敗筆數", "18 筆", "需重試", "down"],
    ["時", "最後同步", "09:08", "2026-07-14", "up"]
  ]
});

Object.assign(rows, {
  adminSync: [
    ["SQL-20260714-001", "業主及客戶資料", "CCPS 業主/租客資料", "128", "126", "2", "同步失敗", "電話格式缺失", "2026-07-14 09:08", "第 2 次"],
    ["SQL-20260714-002", "租金帳單", "租客與租金管理", "86", "86", "0", "已同步", "-", "2026-07-14 09:18", "自動"],
    ["SQL-20260714-003", "房款收款記錄", "建案與房款分期", "39", "37", "2", "待重試", "憑證缺失", "2026-07-14 09:28", "手動"],
    ["SQL-20260714-004", "分期繳費記錄", "分期繳費與付款憑證", "44", "44", "0", "已同步", "-", "2026-07-14 09:38", "自動"],
    ["SQL-20260714-005", "維修及其他支出", "收支與維修管理", "57", "53", "4", "同步失敗", "科目映射未設定", "2026-07-14 09:48", "第 1 次"],
    ["SQL-20260714-006", "收款和付款憑證", "財務確認管理", "72", "70", "2", "同步中", "等待附件校驗", "2026-07-14 09:58", "自動"],
    ["SQL-20260713-011", "預備金流水", "屋主預備金管理", "56", "56", "0", "已同步", "-", "2026-07-13 18:20", "自動"],
    ["SQL-20260713-012", "租金帳單", "租客與租金管理", "92", "91", "1", "待重試", "租約編號缺失", "2026-07-13 18:35", "第 2 次"],
    ["SQL-20260713-013", "維修及其他支出", "收支與維修管理", "33", "33", "0", "已同步", "-", "2026-07-13 18:50", "手動"],
    ["SQL-20260713-014", "房款收款記錄", "建案與房款分期", "28", "27", "1", "同步失敗", "付款日期格式錯誤", "2026-07-13 19:05", "第 1 次"],
    ["SQL-20260712-021", "業主及客戶資料", "CCPS 業主/租客資料", "144", "144", "0", "已同步", "-", "2026-07-12 17:15", "自動"],
    ["SQL-20260712-022", "收款和付款憑證", "財務確認管理", "65", "64", "1", "待重試", "附件路徑無效", "2026-07-12 17:25", "手動"]
  ]
});

Object.assign(headers, {
  adminSync: ["批次號", "同步內容", "來源資料", "筆數", "成功", "失敗", "同步狀態", "失敗原因", "最後同步時間", "重試", "操作"]
});
