<template>
  ...
  <section v-if="financeType === 'property'" class="content-grid admin-finance-workspace">
    <div class="panel table-panel finance-review-panel">
      <div class="panel-head">
        ...
        <div class="finance-list-actions"><span v-if="selectedIds.length">{{ $t('legacy.t_06fb19973447') }} {{ selectedIds.length }} {{ $t('legacy.t_d0bb9b2b8ea7') }}</span><span v-if="loading">{{ $t('legacy.t_6ce3778a43cd') }}</span></div>
      </div>
      ...
      <div v-else class="table-wrap finance-review-table-wrap">
        <table>
          <thead><tr><th class="finance-check-cell"><input type="checkbox" :checked="allPendingSelected" :disabled="!pendingPageRows.length" :aria-label="$t('legacy.t_b944d0833969')" @change="togglePendingPage"></th><th>{{ $t('legacy.t_32e88cf956f6') }}</th><th>{{ $t('legacy.t_114246450ff0') }}</th><th>{{ $t('legacy.t_d61f1eba334a') }}</th><th>{{ $t('legacy.t_293b17eefc95') }}</th><th>{{ $t('legacy.t_058f511c98cf') }}</th><th>{{ $t('legacy.t_e2339796356e') }}</th><th>{{ $t('legacy.t_4feb65fa9683') }}</th><th>{{ $t('legacy.t_b041d1d11b87') }}</th><th>{{ $t('legacy.t_f3ea6d345e2a') }}</th></tr></thead>
          <tbody>
            <tr v-for="row in rows" :key="row.id" :class="{ selected: row.id === selectedId }" @click="selectRow(row)">
              <td class="finance-check-cell"><input v-if="row.confirmationStatus === 'pending'" v-model="selectedIds" type="checkbox" :value="row.id" :aria-label="$t('finance.selectTransaction', { transactionNo: row.transactionNo })" @click.stop></td>
              ...
              <td><div class="finance-document-actions" v-if="viewMode === 'history'"><a :href="documentUrl(row, 'invoice')" download @click.stop>Invoice / 发票</a><a :href="documentUrl(row, 'receipt')" download @click.stop>Official Receipt / 收据</a></div><button v-else type="button" class="row-actions" :title="$t('legacy.t_7eba3bdf9f23')" @click.stop="selectRow(row)">…</button></td>
            </tr>
    ...
    <aside class="panel detail-panel finance-review-detail">
      ...
      <div v-else class="admin-owner-empty">{{ $t('legacy.t_7121ab7d9313') }}</div>
    </aside>
  ...
  <AdminExpenseFinanceWorkspace v-else ref="expenseFinance" />
  </div>
</template>
...
export default {
  components: { AdminRentCollectionWorkspace, AdminRentFinanceWorkspace, AdminReserveFinanceWorkspace, AdminExpenseFinanceWorkspace, ModuleToolbar },
  inject: ['page'],
  data() {
    return {
  ...
  mounted() {
    this.financeType = ['property', 'rent', 'reserve', 'expense'].includes(this.page.adminFinanceMode) ? this.page.adminFinanceMode : 'property';
  ...
  methods: {
    ...
    async batchDownloadDocuments(type) {
      ...
      const ids = this.selectedIds.length ? this.selectedIds : this.rows.map(row => row.id).filter(Boolean);
