    ...
    <div class="rent-income-layout">
      <div class="rent-main-column">
        <form class="rent-filter-card" @submit.prevent="applyFilters">
          <label>
            <span>{{ $t('legacy.t_598a81bde0d4') }}</span>
          ...
          <label>
            ...
            <select v-model.number="draftFilters.year"><option v-for="year in years" :key="year" :value="year">{{ year }}{{ $t('legacy.t_6f14c2641c25') }}</option></select>
          </label>
          <label>
          ...
          <div class="rent-filter-actions">
            ...
            <button class="rent-reset-button" type="button" :disabled="loading" @click="resetFilters">{{ $t('legacy.t_3d81345303ab') }}</button>
          </div>
          <button class="rent-export-button" type="button" :disabled="loading || !filteredRows.length" @click="exportRows">
            <span v-html="icons.download"></span> {{ $t('legacy.t_30c2473f0b44') }} </button>
        </form>

        <section class="rent-table-card">
          <header class="rent-card-heading">
            <div><h2>{{ $t('legacy.t_fb0f404d5bf0') }}</h2><span>{{ $t('legacy.t_3b6ef811b85a') }} {{ filteredRows.length }} {{ $t('legacy.t_7bef4909653a') }}</span></div>
          </header>
          <div class="rent-table-wrap">
            <table class="rent-table">
              <thead>
                <tr><th>{{ $t('legacy.t_598a81bde0d4') }}</th><th>{{ $t('legacy.t_42c950a2ce7c') }}</th><th>{{ $t('legacy.t_a5a8f1ffc0eb') }}</th><th>{{ $t('legacy.t_09f8b2173de4') }}</th><th>{{ $t('legacy.t_77b3e7440615') }}</th><th>{{ $t('legacy.t_a39d7b7ae576') }}</th><th>{{ $t('legacy.t_799d9573b1be') }}</th><th>{{ $t('legacy.t_45293595eae3') }}</th><th>{{ $t('legacy.t_7ce59565a333') }}</th></tr>
              </thead>
              <tbody>
                <tr v-for="row in pagedRows" :key="row.invoiceId">
                  ...
                  <td>
                    ...
                    <button v-else-if="row.canConfirm" class="rent-confirm-button" :disabled="confirmingInvoiceId === row.invoiceId" @click="confirmReceipt(row)">
                      {{ confirmingInvoiceId === row.invoiceId ? $t('legacy.t_03fc655d6daf') : $t('legacy.t_6f242405f405') }}
                    </button>
          ...
          <footer class="rent-table-footer">
            <span>{{ $t('legacy.t_5e4e05716080') }} {{ pageStart }} {{ $t('legacy.t_43401e739ef4') }} {{ pageEnd }} {{ $t('legacy.t_4826afbea9f3') }} {{ filteredRows.length }} {{ $t('legacy.t_9cc6467c8ca1') }}</span>
            <div class="rent-pagination">
              ...
              <select v-model.number="pageSize"><option :value="8">{{ $t('legacy.t_2bb62c39d6fe') }}</option><option :value="10">{{ $t('legacy.t_cb23196e2295') }}</option><option :value="20">{{ $t('legacy.t_c748628a7a99') }}</option></select>
            </div>
          </footer>
        </section>
      </div>
      ...
      <aside class="rent-side-column">
        <section class="rent-side-card rent-trend-card">
          <header><h2>{{ $t('legacy.t_2ff4c7df5463') }}</h2><button @click="showToast($t('legacy.t_68e39da1fcbf'))">{{ $t('legacy.t_7eba3bdf9f23') }} <span>→</span></button></header>
        ...
        <section class="rent-side-card rent-recent-card">
          <header><h2>{{ $t('legacy.t_9c09e1db191c') }}</h2><button @click="showToast($t('legacy.t_8b1bd2526b8a'))">{{ $t('legacy.t_ed2172fd7894') }} <span>→</span></button></header>
          ...
          <footer>{{ $t('legacy.t_d272c76dce30') }}</footer>
        </section>
      </aside>
    </div>
  </section>
