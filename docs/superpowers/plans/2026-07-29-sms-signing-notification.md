...
`docs/短信簽約設定說明.md` 要列出 `SMS_ENABLED`、`SMS_ENDPOINT`、`SMS_API_KEY`、`SMS_SENDER_ID`、`SMS_TIMEOUT_MS`、資料庫 migration 執行順序，以及測試模式不會發真短信的說明。
...
```bash
git add backend/src/test/java/com/ccps/backend/controller/ElectronicSignatureSmsControllerTest.java docs/短信簽約設定說明.md docs/客戶問題每日紀錄.md
