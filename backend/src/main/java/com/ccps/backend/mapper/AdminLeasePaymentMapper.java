...
public interface AdminLeasePaymentMapper {
    @Select("""
        SELECT rp.id AS paymentId,fr.id AS financeRecordId,ri.id AS invoiceId,l.id AS leaseId,l.lease_no AS leaseNo,
          ri.billing_month AS billingMonth,fr.transaction_no AS transactionNo,fr.amount,COALESCE(fr.receipt_date,fr.transaction_date) AS paymentDate,
          COALESCE(fr.receipt_date,fr.transaction_date) AS receivedDate,fr.transaction_date AS postingDate,
    ...
    @Select("""
        SELECT rp.id AS paymentId,fr.id AS financeRecordId,ri.id AS invoiceId,l.id AS leaseId,l.lease_no AS leaseNo,
          ...
          fr.transaction_no AS transactionNo,fr.amount,COALESCE(fr.receipt_date,fr.transaction_date) AS paymentDate,
          COALESCE(fr.receipt_date,fr.transaction_date) AS receivedDate,fr.transaction_date AS postingDate,fr.payment_method AS paymentMethod,
    ...
    @Update("UPDATE rent_payments SET allocated_amount=#{amount} WHERE id=#{paymentId}")
    int updateAllocatedAmount(@Param("paymentId") Long paymentId,@Param("amount") BigDecimal amount);

    @Update("UPDATE finance_records SET amount=#{amount},transaction_date=#{postingDate},receipt_date=#{receivedDate},payment_method=#{paymentMethod},confirmed_by=#{actorId},confirmed_at=NOW() WHERE id=#{financeRecordId} AND payment_status<>'voided'")
    int updateFinance(@Param("financeRecordId") Long financeRecordId,@Param("amount") BigDecimal amount,
            @Param("receivedDate") LocalDate receivedDate,@Param("postingDate") LocalDate postingDate,
    ...
    @Update("UPDATE cashflow_entries SET occurred_on=#{postingDate},description=#{description} WHERE finance_record_id=#{financeRecordId}")
    int updateCashflow(@Param("financeRecordId") Long financeRecordId,@Param("postingDate") LocalDate postingDate,@Param("description") String description);
    ...
    class PaymentRow {
        ...
        private BigDecimal amount,invoiceAmountDue,invoiceAmountPaid;private LocalDate billingMonth,paymentDate,receivedDate,postingDate;private LocalDateTime createdAt;
