...
public interface AdminPropertyPhotoMapper {
    String COLUMNS = "pp.id, pp.owner_unit_id AS ownerUnitId, pp.lease_id AS leaseId,pp.rental_stage AS rentalStage,pp.document_id AS documentId, "
    ...
    @Select("SELECT " + COLUMNS + JOINS + "WHERE pp.owner_unit_id=#{ownerUnitId} AND pp.lease_id IS NULL ORDER BY pp.is_cover DESC, pp.sort_order, pp.id")
    List<PhotoRow> listRegular(@Param("ownerUnitId") Long ownerUnitId);
    ...
    @Select("SELECT " + COLUMNS + JOINS
            ...
            + "ORDER BY pp.is_cover DESC, pp.sort_order, pp.id")
    ...
    @Select("SELECT COUNT(*) FROM property_photos WHERE owner_unit_id=#{ownerUnitId} AND lease_id IS NULL")
    int countRegular(@Param("ownerUnitId") Long ownerUnitId);

    @Select("SELECT COUNT(*) FROM property_photos WHERE owner_unit_id=#{ownerUnitId} AND lease_id IS NULL AND is_cover=1")
    int regularCoverCount(@Param("ownerUnitId") Long ownerUnitId);
    ...
    @Insert("INSERT INTO property_photos (owner_unit_id,lease_id,rental_stage,document_id,title,category,description,sort_order,is_cover) "
            + "VALUES (#{ownerUnitId},#{leaseId},#{rentalStage},#{documentId},#{title},#{category},#{description},#{sortOrder},#{coverFlag})")
    @Options(useGeneratedKeys = true, keyProperty = "id")
    int insertPhoto(PhotoRow row);

    @Update("UPDATE property_photos SET lease_id=#{leaseId},rental_stage=#{rentalStage},title=#{title},category=#{category},description=#{description},sort_order=#{sortOrder},is_cover=#{coverFlag} "
    ...
    @Update("UPDATE property_photos SET is_cover=0 WHERE owner_unit_id=#{ownerUnitId} AND lease_id IS NULL")
    int clearRegularCover(@Param("ownerUnitId") Long ownerUnitId);

    @Update("UPDATE property_photos SET is_cover=1 WHERE id=(SELECT id FROM (SELECT id FROM property_photos WHERE owner_unit_id=#{ownerUnitId} AND lease_id IS NULL ORDER BY sort_order,id LIMIT 1) first_photo)")
    int assignFirstRegularCover(@Param("ownerUnitId") Long ownerUnitId);
