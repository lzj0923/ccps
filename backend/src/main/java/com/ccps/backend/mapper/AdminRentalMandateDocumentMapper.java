...
public interface AdminRentalMandateDocumentMapper {
    @Select("SELECT d.id,d.document_no,d.original_name,d.document_type,dl.relation_type,d.mime_type,d.file_size,d.created_at FROM documents d JOIN document_links dl ON dl.document_id=d.id WHERE dl.entity_type='rental_mandate' AND dl.entity_id=#{mandateId} ORDER BY d.created_at DESC")
    ...
    class AttachmentFile { private String originalName,storageKey,mimeType; private Long fileSize; public String getOriginalName(){return originalName;} public void setOriginalName(String v){originalName=v;} public String getStorageKey(){return storageKey;} public void setStorageKey(String v){storageKey=v;} public String getMimeType(){return mimeType;} public void setMimeType(String v){mimeType=v;} public Long getFileSize(){return fileSize;} public void setFileSize(Long v){fileSize=v;} }
}
