...
public interface AdminRentalMandateMapper {
    String SELECT = """
            SELECT rm.id, rm.mandate_no, rm.owner_unit_id, ou.owner_id, o.full_name AS owner_name,
                   ...
                   rm.responsible_user_id, ru.full_name AS responsible_user_name, rm.status,
                   rm.review_note, rm.termination_reason, rm.submitted_at, rm.reviewed_at,
                   rv.full_name AS reviewed_by_name, rm.created_at
    ...
    @Select("SELECT id, full_name AS name FROM users WHERE status = 'active' ORDER BY full_name")
    List<UserOptionRow> findActiveUsers();

    @Select("SELECT COUNT(*) FROM owner_units WHERE id = #{ownerUnitId} AND status = 'active' AND asset_stage = 'OPERATING'")
    ...
    @Select("SELECT h.id, h.mandate_id, h.from_status, h.to_status, h.reason, h.changed_by, u.full_name AS changed_by_name, h.changed_at FROM rental_mandate_status_history h LEFT JOIN users u ON u.id = h.changed_by WHERE h.mandate_id = #{mandateId} ORDER BY h.changed_at DESC, h.id DESC")
