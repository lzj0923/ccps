...
public class AdminPropertyImportantMessageService {
    ...
    @Transactional(readOnly=true)
    public List<AdminPropertyImportantMessageResponse> listForOwnerUser(Long userId,Long ownerUnitId){requirePropertyForUser(userId,ownerUnitId);return mapper.list(ownerUnitId).stream().map(this::response).toList();}
    @Transactional
