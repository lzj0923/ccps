...
public class AdminRentalMandateService {
    ...
    @Transactional(readOnly = true)
    public Item detail(Long mandateId) { return item(requireMandate(mandateId)); }

    @Transactional
    public Item update(Long actorId, Long mandateId, AdminRentalMandateCreateRequest request) {
        String status = mapper.lockStatus(mandateId);
        MandateRow current = requireMandate(mandateId);
        if (!ACTIVE_STATUSES.contains(status == null ? "" : status)) throw conflict("Only active or suspended mandates can be edited");
        if (!java.util.Objects.equals(current.getOwnerUnitId(), request.ownerUnitId())) throw bad("Cannot change the property of an existing mandate");
        if (!MANDATE_TYPES.contains(request.mandateType())) throw bad("Unsupported rental mandate type");
        if (request.startDate() == null || (request.endDate() != null && request.endDate().isBefore(request.startDate()))) throw bad("Mandate end date cannot be before start date");
        if (request.endDate() != null && request.endDate().isBefore(LocalDate.now())) throw bad("Please terminate the mandate to end an existing mandate");
        if (mapper.countLeasesOutsideTerm(mandateId, request.startDate(), request.endDate()) > 0) throw conflict("Mandate dates must cover existing leases");
        if (mapper.countOtherOverlapping(mandateId, request.ownerUnitId(), request.startDate(), request.endDate()) > 0) throw conflict("An overlapping rental mandate already exists for this property");
        if (request.responsibleUserId() != null && mapper.countActiveUser(request.responsibleUserId()) != 1) throw bad("Responsible user is not active");
        if (mapper.updateDetails(mandateId, request) != 1) throw conflict("Rental mandate was changed by another request");
        // Existing posted fees and signed documents remain historical records, not regenerated by editing.
        mapper.insertHistory(mandateId, status, status, "编辑委托资料（已生成账单及签署文件保持不变）", actorId);
        audit(actorId, "update", mandateId, null, "{\"detailsUpdated\":true}");
    ...
    private MandateRow requireMandate(Long id) {
        MandateRow row = id == null || id <= 0 ? null : mapper.findById(id);
        if (row == null) {
